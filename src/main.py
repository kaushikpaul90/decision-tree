...
def run_cross_validation(X, y, feature_names):
    ...
    for fold, (train_index, test_index) in enumerate(skf.split(X, y), 1):
        ...
        dt_classifier = DecisionTreeClassifier(random_state=SEED, criterion="entropy")
        print(f"Using Decision Tree criterion: {dt_classifier.criterion}")
    ...
    # Minor Improvement: write CSV header comment for clarity
    summary_file_path = os.path.join(SUMMARY_DIR, "predicted_labels_summary.csv")
    with open(summary_file_path, "w", encoding="utf-8") as f:
        f.write("# Predicted labels across all 6 folds\n")
        f.write("# Columns: Sample_Index_in_Original_Dataset, Actual_Label, Predicted_Label, Fold\n")
    predictions_df.to_csv(summary_file_path, mode="a", index=False)
...
def perform_analysis_and_plot(results_df, class_names):
    ...
    # Minor Improvement: write CSV header comment for clarity
    summary_metrics_path = os.path.join(SUMMARY_DIR, "metrics_summary.csv")
    with open(summary_metrics_path, "w", encoding="utf-8") as f:
        f.write("# Mean and Variance of Class-wise Metrics across 6 folds\n")
        f.write("# Columns include Accuracy, Precision, Recall, and Specificity for each class\n")
    analysis_df.to_csv(summary_metrics_path, mode="a")
...
def main():
    ...
    print("\n" + "="*53)
    print("\nAll tasks completed successfully. Summary of outputs:")
    print("\n" + "="*53)
    print("1. Decision Tree images (in output/decision_trees/).")
    print("2. Box Plots image (in output/box_plots/).")
    print("3. Metrics summary (in output/summary_metrics/metrics_summary.csv).")
    print("4. Predicted labels summary (in output/summary_metrics/predicted_labels_summary.csv).")
